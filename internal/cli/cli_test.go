...
func TestRunRejectsAmbiguousShortRPCFilter(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err := Run([]string{"../analyzer/testdata/duplicate", "--rpc", "CreateOrder", "--format", "json"}, &stdout, &stderr)
	if err == nil {
	...
	want := `calltrail-go: ambiguous --rpc "CreateOrder" matched 2 handlers; use one of: debugService.CreateOrder, userService.CreateOrder`
...
func TestRunAllowsReceiverQualifiedRPCFilter(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err := Run([]string{"../analyzer/testdata/duplicate", "--rpc", "userService.CreateOrder", "--format", "json"}, &stdout, &stderr)
	if err != nil {
		t.Fatalf("Run returned error: %v\nstderr:\n%s", err, stderr.String())
	}
	if stderr.Len() != 0 {
	...
	if flows[0].Entrypoint.Symbol != "userService.CreateOrder" {
		t.Fatalf("entrypoint symbol = %q, want userService.CreateOrder", flows[0].Entrypoint.Symbol)
	}
}
